<div class="group relative bg-[var(--surface-color)] border border-transparent hover:border-[var(--border-color)] transition-all duration-500 p-2 cursor-pointer">
    
    <a href="{{ route('product.detail', $product->id) }}" class="absolute inset-0 z-10" aria-label="View {{ $product->name }}"></a>
    ...
    <div class="relative w-full aspect-[4/5] overflow-hidden bg-[#F3EFEC]">
        ...
        <div class="absolute top-4 right-4 flex flex-col gap-2 opacity-0 translate-y-2 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out z-30">
            <button class="wishlist-btn w-9 h-9 flex items-center justify-center bg-white/80 backdrop-blur-sm text-[var(--primary-color)] hover:bg-[var(--primary-color)] hover:text-white transition-all shadow-sm">
        ...
        <div class="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out z-30">
            <a href="{{ route('product.detail', $product->id) }}" class="w-full block py-4 text-center text-xs font-bold uppercase tracking-widest text-white bg-[var(--primary-color)] hover:bg-[var(--primary-hover)] transition-colors">
                Quick Add to Cart
            </a>
        </div>
    </div>
    ...
    <div class="mt-6 px-2 pb-4 text-center">
        ...
        <h3 class="text-lg font-serif italic text-[var(--text-on-secondary)] mb-2 relative z-20">
            {{ $product->name }}
        </h3>

        <div class="mt-2 text-[12px] leading-tight text-gray-500 line-clamp-2 overflow-hidden mb-3 relative z-20">
    {!! $product->description !!}
</div>
        <div class="flex items-center justify-center gap-3 relative z-20">
        ...
        @if($product->rating)
        <div class="flex items-center justify-center gap-1 mt-3 relative z-20">
