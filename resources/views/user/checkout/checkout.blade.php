@extends('user.layouts.master-layouts.plain')
    <title>Footwear Premium - Checkout</title>
...
@foreach($cartItems as $item)
    @php
        ...
        $image = $product->defaultImage->image_path 
    ...
    <div class="flex items-center">
        <div class="w-12 h-12 bg-[var(--primary-color)]/10 rounded-lg flex items-center justify-center mr-3">
            ...
            <div class="flex-grow">
                ...
                @if($item->size)
    <p class="text-xs text-gray-500">Size: {{ $item->size->name }}</p>
@endif

@if($item->color)
    <p class="text-xs text-gray-500">Color: {{ $item->color->name }}</p>
@endif
...
@push('script')
    <script>
        ...
        document.getElementById('same-billing').addEventListener('change', function() {
            if (this.checked) {
                console.log('Billing address will be same as shipping');
            } else {
                console.log('Billing address fields would be shown here');
            }
        });
    </script>
@endpush